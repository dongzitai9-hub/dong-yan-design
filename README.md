...
- 页面改版时采用“替换旧规则”，不要用新 class 长期覆盖旧 class。已经确定不用的旧背景色、旧导航、旧模块、旧文案、旧图片路径和旧样式，应从源代码或正式资源中删除或替换。
- 竖向信息图不适合直接左右补大面积底色做满屏横图；如果要用于第二屏这类横向满屏叙事图，应在不改变原有信息内容和结论的前提下，重新做横向构图。原竖图可继续用于图文段或手机端。
...
- 目录类页面主背景固定使用浅灰白 `#F2F3F4`，不要沿用旧的偏米色列表背景。
...
- 厨房类札记如果采用“满屏屏幕流”，优先按“案例式首屏、整屏叙事图、旧背景、现代系统问答、布局、U 型、台面分区、尺寸、高度、总结”组织，不默认回到居中文章版式。
- 札记案例式第二屏叙事图参考 `/cases/space-015/`：整屏 `100vh`，上方留白 `40px`，图片高度 `calc(100vh - 40px - 120px - 30px)`，图片下方空白 `120px`，底部文字高度 `30px`，文字字号 `30px` 并居中；图片铺满宽度时使用 `object-fit: cover`。
- 竖向信息图如果要放进电脑端横向叙事图区域，先裁取图片内部白底内容，再用同色白底扩成横向画布；不能裁掉标题、标注或结论文字。手机端优先保留原竖图完整阅读，不强行使用横向扩图。
